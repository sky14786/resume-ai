...
> 글쓰기 스타일 규칙(AI 투 표현, 구분자 등)은 `CLAUDE.md` 참고. 이 문서는 사실/내용만 다룬다.
...
2. 고객사 담당자와 1:1 소통 (현지 출장, AI 번역 활용), 판단 가능한 건 선조치 후 보고
...
4. 수집 지표 기준치 설정 UI 개발 (발송 API는 서버에서 제공)
...
- **부제**: 입사 후 첫 프로젝트로 어드민 시스템 구현 담당
- **스택**: Spring Boot, JSP, jQuery, MySQL
- **통계**: 6개월 / 6개 기능

**불릿**
1. 설계된 어드민 MVC 아키텍처를 Spring Boot + JSP로 구현
...
**불릿**
1. 게임 아이템 매매 게시판, 쪽지함, 관리자 패널 전체 기능 설계/구현
