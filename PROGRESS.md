...
### 💬 검토 중인 아이디어
- [ ] **각 프로젝트별 아키텍처 다이어그램 삽입 가능 여부**
  - 질문: 병원 앱, IPTV, WACS 각각 시스템 구성도를 그려서 카드 내에 넣을 수 있나?
  - 방법 후보:
    - SVG로 직접 그려서 HTML에 인라인 삽입
    - Mermaid.js (CDN) — 코드로 다이어그램 생성
    - 이미지 파일로 export 후 `<img>` 태그
  - 고려사항: 카드 hover 시 아키텍처 펼쳐지는 인터랙션 가능
